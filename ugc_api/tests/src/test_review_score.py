...
REVIEW_ID = "61366713518a6389d3ed7ee7"
...
@pytest.mark.asyncio
async def test_get_review(headers, make_get_request):
    data = {
        "_id": REVIEW_ID,
        "movie_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
        "text": "New text.",
...
async def test_add_review_score(headers, make_post_request, make_get_request):
    ...
    data = {"review_id": REVIEW_ID, "score": 10}
    response = await make_post_request(
        f"{API_URL}/",
        data=data,
        headers=headers
    )
    assert response.status == 200, "Couldn't add review score."
...
@pytest.mark.asyncio
async def test_get_review_score(headers, make_post_request, make_get_request):
    response = await make_get_request(f"{API_REVIEWS_URL}/{REVIEW_ID}",
                                      headers=headers)
    assert response.status == 200, "Couldn't get review."
    assert response.body["scores_quality"] != 0, "Review doesn't have scores"

    review_score_id = response.body["scores"][0]
    response = await make_get_request(f"{API_URL}/{review_score_id}",
                                      headers=headers)
    assert response.status == 200, "Couldn't get review score."
